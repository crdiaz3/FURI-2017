...
def write_to_csv(data,file_name): #reference: http://pymotw.com/2/csv/
  with open(file_name,'w') as f: #existing file with the same name will be overwritten
    ...
    writer.writerow( ('Rank', 'Population Growth Rate', 'City', 'State', 'Population') )
...
def get_land_info():
    ...
    base_url = "http://www.usa.com/rank/us--population-growth-rate--city-rank.htm?hl=&hlst=&wist=&yr=9000&dis=50&sb=DESC&plow=&phigh=&ps="
    ...
    for page_num in range(1,148):
        ...
        for city in row_data:
            ...
            if rank == "Rank":
                ...
                growth_rate = city_data[1].string
                ...
                if len(city_data[2].contents) > 1:
                    ...
                    captured_data.append([rank,growth_rate,city,state,pop])
                else:
                    pop = '0'
                    captured_data.append([rank,growth_rate,city,state,pop])

        write_to_csv(captured_data,"pop-growth-rate-data.csv")

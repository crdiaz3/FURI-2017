...
def get_land_info():
    ...
    base_url = "http://www.usa.com/rank/us--crime-index--city-rank.htm?hl=&hlst=&wist=&yr=9000&dis=50&sb=DESC&plow=&phigh=&ps="
    ...
    for page_num in range(1,148):
        ...
        for city in row_data:
            ...
            if rank == "Rank":
                ...
                crime_index = city_data[1].string
                ...
                if len(city_data[2].contents) > 1:
                    ...
                    captured_data.append([rank,crime_index,city,state,pop])
                else:
                    pop = '0'
                    captured_data.append([rank,crime_index,city,state,pop])

        write_to_csv(captured_data,"crime-index-data.csv")

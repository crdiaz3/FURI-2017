...
def write_to_csv(data,file_name): #reference: http://pymotw.com/2/csv/
  with open(file_name,'w') as f: #existing file with the same name will be overwritten
    ...
    writer.writerow( ('Rank', 'Water Area (sq mi)', 'City', 'State', 'Population') )
...
def get_water_info():
    ...
    base_url = "http://www.usa.com/rank/us--water-area--city-rank.htm?hl=&hlst=&wist=&yr=9000&dis=50&sb=DESC&plow=&phigh=&ps="
    ...
    for page_num in range(1,148):
        ...
        for city in row_data:
            ...
            if rank == "Rank":
                ...
                water_area = city_data[1].string.replace(' sq mi','')
                ...
                if len(city_data[2].contents) > 1:
                    ...
                    captured_data.append([rank,water_area,city,state,pop])
                else:
                    pop = '0'
                    captured_data.append([rank,water_area,city,state,pop])

        write_to_csv(captured_data,"water-area-data.csv")
